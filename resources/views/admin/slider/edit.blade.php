                                ...
                                <div class="form-group">
                                    ...
                                    <select id="position" name="position"
                                        ...
                                        @for ($i = 1; $i <= count(\App\Enums\SliderPosition::getKeys()); $i++)
                                            <option value="{{$i}}" {{$slider->position == $i ? 'selected' : ''}}>{{\App\Enums\SliderPosition::get($i)}}</option>
                                        @endfor
                                    </select>
                                    @error('position')
                                    @include('admin.components.validation')
                                ...
                                <div class="form-group">
                                    ...
                                    <select id="sliderStatus" name="status"
                                        ...
                                        @for ($i = 1; $i <= count(\App\Enums\SliderStatus::getKeys()); $i++)
                                            <option value="{{$i}}" {{$slider->status == $i ? 'selected' : ''}}>{{\App\Enums\SliderStatus::get($i)}}</option>
                                        @endfor
                                    </select>
                                    @error('status')
                                    @include('admin.components.validation')
                                ...
                                <div class="form-group">
                                    ...
                                    <select id="type" name="type"
                                        ...
                                        @for ($i = 1; $i <= count(\App\Enums\SliderType::getKeys()); $i++)
                                            <option value="{{$i}}" {{$slider->type == $i ? 'selected' : ''}}>{{\App\Enums\SliderType::get($i)}}</option>
                                        @endfor
                                    </select>
                                    @error('type')
                                    @include('admin.components.validation')
                                ...
                                <div class="form-group">
                                    ...
                                    <select id="advertise_id" class="form-control select2" name="advertise_id"
                                        ...
                                        @foreach($advertises as $advertise)
                                            <option value="{{$advertise->id}}" {{$slider->advertise_id == $advertise->id ? 'selected' : ''}}>{{$advertise->title}}</option>
                                        @endforeach
                                    </select>
                                    @error('course_id')
                                    @include('admin.components.validation')
                                ...
                                <div class="form-group">
                                    ...
                                    <select id="screen" name="screen"
                                        ...
                                        @for ($i = 1; $i <= count(\App\Enums\Screen::getKeys()); $i++)
                                            <option value="{{\App\Enums\Screen::getKey($i)}}" {{$slider->screen == \App\Enums\Screen::getKey($i) ? 'selected' : ''}}>{{\App\Enums\Screen::get($i)}}</option>
                                        @endfor
                                    </select>
                                    @error('screen')
                                    @include('admin.components.validation')
                            ...
                            <div class="col-md-12" id="linkdiv" style="display: none;">
                                <div class="form-group">
                                    ...
                                    <input type="text" placeholder="Example:https://aramia.me/" id="link"
                                           class="form-control englishText" value="{{$slider->link}}"
                                           name="link">
                                    @error('link')
                                    @include('admin.components.validation')
                                    @enderror
